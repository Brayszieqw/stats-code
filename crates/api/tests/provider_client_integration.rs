...
#[test]
fn provider_client_routes_gpt_aliases_through_openai() {
    let _lock = env_lock();
    let _openai_api_key = EnvVarGuard::set("OPENAI_API_KEY", Some("openai-test-key"));

    let client = ProviderClient::from_model("gpt").expect("gpt alias should resolve");
...
fn provider_client_routes_gemini_aliases_through_gemini() {
    ...
    let client =
        ProviderClient::from_model("gemini").expect("gemini alias should resolve to Gemini");
...
#[test]
fn provider_client_reports_missing_openai_credentials_for_gpt_models() {
    let _lock = env_lock();
    let _openai_api_key = EnvVarGuard::set("OPENAI_API_KEY", None);

    let error = ProviderClient::from_model("gpt-5.4")
        .expect_err("gpt requests without OPENAI_API_KEY should fail fast");
...
#[test]
fn provider_detection_prefers_openai_before_anthropic_without_explicit_model() {
    let _lock = env_lock();
    let _openai_api_key = EnvVarGuard::set("OPENAI_API_KEY", Some("openai-test-key"));
    ...
    let client = ProviderClient::from_model("custom-analysis-model")
        .expect("fallback provider should prefer OpenAI first");
