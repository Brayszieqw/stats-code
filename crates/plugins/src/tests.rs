...
fn script_path(root: &Path, directory: &str, stem: &str) -> PathBuf {
    root.join(directory).join(format!("{stem}.{SCRIPT_EXT}"))
}

fn script_ref(directory: &str, stem: &str) -> String {
    format!("./{directory}/{stem}.{SCRIPT_EXT}")
}

fn write_script(path: &Path, unix_contents: &str, windows_contents: &str) {
    #[cfg(windows)]
    let contents = windows_contents;
    #[cfg(not(windows))]
    let contents = unix_contents;
    #[cfg(windows)]
    let _ = unix_contents;
    #[cfg(not(windows))]
...
fn write_loader_plugin(root: &Path) {
    write_script(
        script_path(root, "hooks", "pre").as_path(),
        "#!/bin/sh\nprintf 'pre'\n",
        "@echo off\r\n<nul set /p dummy=pre\r\n",
    );
    write_script(
        script_path(root, "tools", "echo-tool").as_path(),
        "#!/bin/sh\ncat\n",
        "@echo off\r\nmore\r\n",
    );
    write_script(
        script_path(root, "commands", "sync").as_path(),
        "#!/bin/sh\nprintf 'sync'\n",
        "@echo off\r\n<nul set /p dummy=sync\r\n",
    );
    write_file(
  ...
  "commands": [
    {{
      ...
      "command": "{}"
            ...
            script_ref("hooks", "pre"),
            script_ref("tools", "echo-tool"),
            script_ref("commands", "sync")
        )
        .as_str(),
    );
}
...
fn write_external_plugin(root: &Path, name: &str, version: &str) {
    ...
    write_script(
        script_path(root, "hooks", "post").as_path(),
        "#!/bin/sh\nprintf 'post'\n",
        "@echo off\r\n<nul set /p dummy=post\r\n",
    );
    write_file(
...
fn write_lifecycle_plugin(root: &Path, name: &str, version: &str) -> PathBuf {
    ...
    write_script(
        script_path(root, "lifecycle", "init").as_path(),
        "#!/bin/sh\nprintf 'init\\n' >> lifecycle.log\n",
        "@echo off\r\necho init>> lifecycle.log\r\n",
    );
    write_script(
        script_path(root, "lifecycle", "shutdown").as_path(),
        "#!/bin/sh\nprintf 'shutdown\\n' >> lifecycle.log\n",
    ...
    write_file(
            ...
            format!(
                ...
                script_ref("lifecycle", "init"),
                script_ref("lifecycle", "shutdown")
            )
            .as_str(),
        );
    log_path
...
fn write_tool_plugin_with_name(root: &Path, name: &str, version: &str, tool_name: &str) {
    write_script(
            script_path(root, "tools", "echo-json").as_path(),
...
fn installs_enables_updates_and_uninstalls_external_plugins() {
    ...
    assert!(hooks.pre_tool_use[0].contains(&format!("pre.{SCRIPT_EXT}")));
