...
describe('EquivalentCodeSidecar â€?default tab', () => {
  it('activates the R tab on first render', async () => {
    ...
    // R tab is selected from the very first render â€?assert before the
...
describe('EquivalentCodeSidecar â€?tab switching', () => {
...
describe('EquivalentCodeSidecar â€?none coverage', () => {
...
describe('EquivalentCodeSidecar â€?sidecar_only coverage', () => {
...
describe('EquivalentCodeSidecar â€?live coverage', () => {
...
describe('EquivalentCodeSidecar â€?footer', () => {
  it('renders SHA256 + release version on every tab and every state', async () => {
    ...
    // R is `none` â†?placeholder. Footer must still be present.
    await waitFor(() => {
      expect(screen.getByTestId('sidecar-placeholder')).toBeInTheDocument();
    });
    {
    ...
    // SAS â†?recorded snippet. Footer still present.
    act(() => {
    ...
    // SPSS â†?sidecar_only. Footer still present.
    act(() => {
...
describe('EquivalentCodeSidecar â€?lazy fetching', () => {
